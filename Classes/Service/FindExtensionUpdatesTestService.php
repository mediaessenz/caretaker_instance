 ...
 *
 */
class FindExtensionUpdatesTestService extends RemoteTestServiceBase
{
    /**
     * Value Description
     *
     ...
     * @var string
     */
    protected $typeDescription = 'LLL:EXT:caretaker_instance/Resources/Private/Language/locallang.xml:find_extension_updates_test_description';
     ...
     * @var string
     */
    protected $configurationInfoTemplate = 'LLL:EXT:caretaker_instance/Resources/Private/Language/locallang.xml:find_extension_updates_test_configuration';

    /**
     * Execute the find insecure extension test
     *
     * @return \tx_caretaker_TestResult
     */
    public function runTest()
    {
        ...
        if (!$extensionListResult->isSuccessful()) {
            return \tx_caretaker_TestResult::create(\tx_caretaker_Constants::state_error, 0, 'Remote operation failed: ' . $extensionListResult->getValue());
        }
        ...
        if (!$this->isTYPO3VersionIgnored()) {
            ...
            if (!$typo3VersionResult->isSuccessful()) {
                return \tx_caretaker_TestResult::create(\tx_caretaker_Constants::state_error, 0, 'Remote operation failed: ' . $typo3VersionResult->getValue());
            }
        ...
        if ($num_errors > 0) {
            $submessages[] = new \tx_caretaker_ResultMessage('LLL:EXT:caretaker_instance/Resources/Private/Language/locallang.xml:find_extension_updates_test_detail_error');
            foreach ($errors as $error) {
                $submessages[] = new \tx_caretaker_ResultMessage($error['message'], $error['values']);
            }
        }

        // add warning submessages
        if ($num_warnings > 0) {
            $submessages[] = new \tx_caretaker_ResultMessage('LLL:EXT:caretaker_instance/Resources/Private/Language/locallang.xml:find_extension_updates_test_detail_warning');
            foreach ($warnings as $warning) {
                $submessages[] = new \tx_caretaker_ResultMessage($warning['message'], $warning['values']);
            }
        }

        // add ok submessages
        if ($num_oks > 0) {
            $submessages[] = new \tx_caretaker_ResultMessage('LLL:EXT:caretaker_instance/Resources/Private/Language/locallang.xml:find_extension_updates_test_detail_ok');
            foreach ($oks as $ok) {
                $submessages[] = new \tx_caretaker_ResultMessage($ok['message'], $ok['values']);
            }
        }
        ...
        if ($num_errors > 0) {
            ...
            $message = new \tx_caretaker_ResultMessage('LLL:EXT:caretaker_instance/Resources/Private/Language/locallang.xml:insecure_extension_test_problems', $values);

            return \tx_caretaker_TestResult::create(\tx_caretaker_Constants::state_error, $value, $message, $submessages);
        }
        ...
        if ($num_warnings > 0) {
            ...
            return \tx_caretaker_TestResult::create(\tx_caretaker_Constants::state_warning, $value, $message, $submessages);
        }

        // return ok
        $value = 0;
        $message = new \tx_caretaker_ResultMessage('LLL:EXT:caretaker_instance/Resources/Private/Language/locallang.xml:insecure_extension_test_ok', $values);

        return \tx_caretaker_TestResult::create(\tx_caretaker_Constants::state_ok, $value, $message, $submessages);
    }
    ...
    public function checkExtension($extension, &$errors, &$warnings, &$oks, $typo3Version = '')
    {
        ...
        // Ext is in TER
        if ($ter_info) {
            $message = 'LLL:EXT:caretaker_instance/Resources/Private/Language/locallang.xml:find_extension_updates_test_detailinfo';
            $value = [
                'ext_key' => $extension['ext_key'],
                'ext_version' => $extension['version'],
