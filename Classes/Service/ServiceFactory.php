...
use Caretaker\CaretakerInstance\Command\CommandService;
use Caretaker\CaretakerInstance\Command\RemoteCommandConnector;
use Caretaker\CaretakerInstance\Operation\OperationInterface;
use Caretaker\CaretakerInstance\Operation\OperationManager;
use Caretaker\CaretakerInstance\Security\OpenSSLCryptoManager;
use Caretaker\CaretakerInstance\Security\SecurityManager;
            ...
            if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['caretaker_instance']['operations'])) {
                foreach ($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['caretaker_instance']['operations'] as $key => $operationRef) {
                    if (is_string($operationRef)) {
                        $operation = GeneralUtility::getUserObj($operationRef);
                    } elseif ($operationRef instanceof OperationInterface) {
